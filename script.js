...
function cookCoffee(price, name, elem) {
  ...
  if (balance.value >= price) {
    balance.value -= price;
    balance.style.backgroundColor = ""; //Вернуть белый фон, когда баланс положительный
...
setTimeout(function() {
  ...
  console.log("Timeout and interval are cleared");
}, 1000); */
let cup = {
...
function changeProgress(persent, sec = 0) { //Устанавливаем изначальное значение необязательного параметра
...
for (let bill of bills) {
  bill.onmousedown = dragMoney; //onmousedown - возникновение по клику, еще до отжатия кнопки мыши
}
...
function dragMoney(event) { //Все слушатели события возвращают в функцию первым параметром объект event
  ...
  console.log( this.getBoundingClientRect() ); //Получаем точные координаты расположения выбранного элемента
  ...
  bill.style.transform = "rotate(90deg)"; //Разворот купюры
...
function createCoin(nominal) {//"1", "2", "5", "10"
  ...
  let coin = document.createElement("img"); //создает элемент в "вакууме"
  coin.src = imageSrc;
  coin.style.cursor = "pointer";
  coin.style.userSelect = "none";
  coin.style.width = "30px";
  coin.style.position = "absolute"; //т.к. у родителя (.change-box) значение position задано как relative, то отсчет координат ведется от края родительского элемента
